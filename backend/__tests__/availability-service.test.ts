...
// ═══════════════════════════════════════════════════════════════════════════
describe("getAvailability — modèle `slots` précréés retiré (#31)", () => {
  const input = {
    proId: 1,
    serviceIds: [10],
    fromDate: "2026-09-07",
  ...
  it("uses_availability_engine=false : la dispo est quand même calculée depuis working_hours", async () => {
    installFixture({ pro: { uses_availability_engine: false }, workingHours: MON_9_18 });
    const res = await getAvailability(input);
    expect(res.days[0].slots.length).toBe(33); // 09:00–18:00 par pas de 15 min
  });

  it("uses_availability_engine=false sans working_hours : aucun créneau", async () => {
    installFixture({ pro: { uses_availability_engine: false }, workingHours: [] });
    const res = await getAvailability(input);
    expect(res.days[0].slots).toEqual([]);
  });
});
